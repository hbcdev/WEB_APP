...
@Component({
  selector: 'app-fax-claim',
  templateUrl: './fax-claim.component.html',
  styleUrls: ['./fax-claim.component.css']
})
export class FaxClaimComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.loading = true;
    this.service.Guard();
    this.cookieService.set('menu', '/faxcliam');
  ...
  getDataAssess(data) {
    let call = this._service.get('claim', '/smg', data, 'fax').subscribe((res) => {
